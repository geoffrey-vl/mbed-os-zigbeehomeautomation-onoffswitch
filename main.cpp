...
RawSerial xbee(PC_10, PC_11, 9600);                                                 //Set Serial to XBee
Serial pc(USBTX, USBRX, 9600);                                                //Set Serial to PC.
DigitalOut led2 (LED2);                                                 //configure onboard LEDs
DigitalOut led1 (LED1); 
DigitalOut LED (LED4);        
volatile char x,y,z,len,sum;                                            //some useful internal variables
volatile int length=0, size=0, check=0;
volatile int flag;
char packet[60];
char PacketAddr[10];
char Opacket[60];
